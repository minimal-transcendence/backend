    ...
    if (room === undefined) {
      ...
      if (pwExist) {
        if (room.isPassword(password)) {
          if (room.isBanned(userId)) client.emit('youAreBanned', roomname);
          else
            this.userJoinRoomSuccess(
              io,
              this.storeUser.findUserById(userId),
  ...
  userLeaveRoom(io: Server, userId: number, roomname: string) {
    ...
    if (room.userlist.has(userId)) {
      if (room.userlist.size == 1) {
        room.clearRoom();
        console.log('room  is going to erase');
        this.storeRoom.deleteRoom(roomname);
        console.log(this.storeRoom.rooms.size);
        return;
      }
