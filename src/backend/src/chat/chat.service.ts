...
import {
  ...
  Message,
  DM,
} from '../store/store.message.service';
...
export class ChatService {
  ...
  sendChat(io: Server, client: Socket, to: string, body: string) {
    ...
    if (room.userlist.has(client.data.id)) {
      //이렇게 아이디를 잘 가져올 수 있는지 생각해보자(auth 올라가면 그냥 client.id로 꺼내도 됨 (이건 사실 nickname도 그렇다))
      if (!room.isMuted(client.data.id)) {
        ...
        room.messages.push(message);
      } else client.emit('sendAlert', 'Attention', `You are MUTED in ${to}`);
    } else {
  ...
  userLeaveRoom(io: Server, userId: number, roomname: string) {
    ...
    if (room.userlist.has(userId) && thisUser.joinlist.has(roomname)) {
      ...
      if (room.isOwner(userId)) {
        console.log(
          'userLeaveRoom : enter --------------------------------------------3',
        );
        const newOwner = room.userlist.values().next().value;
        room.updateOwner(newOwner); //되는지 체크 : 아무나 owner로 올림!
      }
      if (room.isOperator(userId)) room.deleteUserFromOperators(userId); //처음에는 owner는 owner역할만 하지만, 첫 owner가 나갈 때  새로  들어오는  newOwner는 중복일 수  있음
      console.log(
        'userLeaveRoom : enter --------------------------------------------4',
      );
      thisUser.joinlist.delete(roomname);
      room.deleteUserFromUserlist(userId);
      ...
      console.log(
        'roomname : ' +
          roomname +
          'room members : ' +
          JSON.stringify(this.makeRoomUserInfo(roomname)),
      );
      io.to('roomname').emit(
        'sendRoomMembers',
        this.makeRoomUserInfo(roomname),
      );
      console.log(
        'userLeaveRoom : enter --------------------------------------------5',
      );
    } else console.log('you are not joining in this room : try leave');
  }

  async userLeaveRoomAct(io: Server, client: Socket, roomname: string) {
    console.log(
      `client ${client.data.nickname} leave roomname : ${roomname} ACT`,
    );
    ...
    console.log(
      'userLeaveRoom : enter --------------------------------------------0',
    );
    this.userJoinRoomAct(
      io,
      this.storeUser.findUserById(client.data.id),
      'DEFAULT',
    );
  }
  ...
  async kickUser(io: Server, roomname: string, targetId: number) {
    ...
    if (room.isOperator(targetId)) room.deleteUserFromOperators(targetId);
    ...
    sockets.forEach((socket) => {
      ...
      socket.emit(
        'sendAlert',
        'Attention',
        `You are kicked out from ${roomname}`,
      );
  ...
  getQueryRoomList(query: string | null): queryResponseRoomInfo[] {
    const res = [];
    if (query === null || query.length === 0) return res;
    ...
    roomlist.forEach((roomname) => {
      ...
      const userlist = Array.from(room.userlist, (user) =>
        this.storeUser.getNicknameById(user),
      );
  ...
  fetchDM(io: Server, from: number, to: number, body: string) {
    ...
    io.to(`$${from}`)
      .to(`$${to}`)
      .emit('sendAlert', 'New DM', 'You got 1 new message');
  }
  ...
  async sendActResultToTarget(
    ...
    let notice: string;
    if (operation === 'kick') notice = 'Kicked out';
    else if (operation === 'ban') notice = 'Banned';
    else if (operation === 'mute') notice = 'Muted';
    const body = `You are ${notice} from Room "${roomname}"`;
    await this.emitEventsToAllSockets(
      io,
      target,
      'sendMessage',
      'server',
  ...
  checkActValidity(roomname: string, actor: number, target: number): boolean {
    ...
    if (target === -1) {
      ...
      //가능하면 owner랑 operator를 enum으로 만들어서 값 비교로 권한 우위 확인하면 더 좋았을듯.... 지금은 귀찮아...
