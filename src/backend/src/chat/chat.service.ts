...
export class ChatService {
  ...
  sendChat(io: Server, client: Socket, to: string, body: string) {
    ...
    if (room.userlist.has(client.data.id)) {
      ...
      client
        ...
        .emit(
          'sendMessage',
          this.storeUser.getNicknameById(client.data.id),
          body,
        ); //방에 emit(본인은 안 받아야)
  ...
  getQueryRoomList(query: string): queryResponseRoomInfo[] {
  ...
  async sendActResultToTarget(
    ...
    await this.emitEventsToAllSockets(io, target, 'sendMessage', [
      'server, body',
    ]);
