  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  constructor(
    ...
    client.on('sendChatMessage', (to, body) => {
      console.log('sendChatmessage server', to, body); //0814
    ...
    client.on('kickUser', (roomname, user) => {
      ...
      console.log('target name: ' + user + 'target Id' + targetId);
      if (
    ...
    client.on('requestSearchResultRoomList', (query) => {
      if (!query) console.log('Query is null');
      const roomInfo = this.chatService.getQueryRoomList(query);
      console.log(
        `query <${JSON.stringify(query, null, 2)}> roomInfo  <${JSON.stringify(
          roomInfo,
          null,
          2,
        )}>`,
