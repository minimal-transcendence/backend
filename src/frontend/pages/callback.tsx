...
function Callback() {
	...
	async function sendAuthCode(code: string) {
		...
		try{
			...
			const detailResponse = await(await fetch('http://localhost/api/user/' + localStorage.getItem('id'))).json();
			...
			{showCodeInput === true && (
				<div className={styles.innerBox}>
					<div className={styles.title2}>
						OTP 인증번호 6자리를 입력해주세요
					</div>
					<div>
						<input className={styles.input} placeholder="띄워쓰기 제외한 6자리" type="text" value={verCode} onChange={(e) => setVerCode(e.target.value)} />
					</div>
					<div>
						<button className={styles.button} onClick={() => sendAuthCode(verCode)}>인증</button>
					</div>
				</div>
			)}
		</div>
