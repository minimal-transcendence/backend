...
function UserProfile(props:any){
	...
	const reloadData = async() => {
		...
		const responseDetail = await axiosApi.get('http://localhost/api/user/' + props.id, );
        const detailResponse = responseDetail.data;
        const responseMatch = await axiosApi.get('http://localhost/api/user/' + props.id + '/matchhistory', );
		...
		return(
			...
			<div className={styles_profile.profileInner}>
				...
				<div className={styles_profile.buttons}>
					{userData[0].nickname !== userNickname && userData[0].isFriend === 0 && (
					<button className={styles_profile.followButton} onClick={() => {follow(0)}}> 팔로우 </button>)}
					{userData[0].nickname !== userNickname && userData[0].isFriend === 1 && (
					<button className={styles_profile.followingButton} onClick={() => {unFollow(0)}}> 언팔로우 </button>)}
	...
	{
		return(
            <div className={styles_profile.fontSet}>
               {getDetailProfile()}
            </div>
		)
	}
